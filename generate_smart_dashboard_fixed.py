#!/usr/bin/env python3
"""
Generate Smart Dashboard with Fixed Warnings and Complete Token Counting
- Removes incorrect stale data warnings
- Includes ALL session types (Claude Desktop + Claude Code)
- Rate limit monitoring at the top
- Smart activity-based updates
"""
...
def get_database_hash(db_path):
    ...
    try:
        ...
        cursor.execute("""
            SELECT id, start_time, end_time, real_total_tokens, billable_tokens, last_token_update
            FROM real_sessions 
...
def get_complete_session_data(db_path):
    """Get complete session data including ALL session types (Claude Desktop + Claude Code)"""
    try:
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
        ...
        # Get total tokens from ALL sessions (Claude Desktop + Claude Code + any others)
        cursor.execute("""
            SELECT 
                ...
                COUNT(CASE WHEN is_active = 1 OR end_time IS NULL THEN 1 END) as active_sessions
            FROM real_sessions
        """)
        ...
        cursor.execute("""
            SELECT 
                session_type,
                COUNT(*) as count,
                SUM(real_total_tokens) as tokens,
        ...
        if latest_session:
            session_data = {
                ...
                "realTokens": totals[0] or 0,  # Use TOTAL from all sessions
                "inputTokens": latest_session[5] or 0,  # real_input_tokens from latest
                "outputTokens": latest_session[6] or 0, # real_output_tokens from latest
                "billableTokens": totals[1] or 0, # Use TOTAL billable from all sessions
                "cacheCreationTokens": totals[3] or 0, # Total cache creation
                "cacheReadTokens": totals[4] or 0, # Total cache read
                "totalCacheTokens": (totals[3] or 0) + (totals[4] or 0),
                "costEstimate": totals[2] or 0.0, # Use TOTAL cost from all sessions
                "modelUsed": latest_session[11] or "Unknown", # models_used from latest
                "isActive": bool(latest_session[12]) if latest_session[12] is not None else False,
                "activeSessionCount": totals[6] or 0
            }
        else:
            session_data = {
        ...
        # Add global totals and session type breakdown
        if totals:
            session_data["globalTotals"] = {
                "totalRealTokens": totals[0] or 0,
                "totalBillableTokens": totals[1] or 0,
                "totalCost": totals[2] or 0.0,
                "totalSessions": totals[5] or 0,
                "activeSessions": totals[6] or 0
            }
            
        # Add session type breakdown
        session_data["sessionTypeBreakdown"] = [
            {
                "type": row[0],
                "sessionCount": row[1],
                "realTokens": row[2] or 0,
                "billableTokens": row[3] or 0,
                "cost": row[4] or 0.0
            }
        ...
        return {
            ...
            "dataSource": "complete_session_data_all_types"
        }
...
def generate_smart_dashboard():
    ...
    print("🔄 Generating smart dashboard with complete token counting...")
    
    # Get real data using improved method
    db_path = "./session_tracker/claude_usage.db"
    session_data = get_complete_session_data(db_path)
    ...
    embedded_data = {
        ...
        "dataSource": "smart_dashboard_complete_tokens_fixed_warnings"
    }
    
    print(f"📊 Session: {session_data['sessionData']['activeSessionId'][:20]}...")
    print(f"💰 Total Cost: ${session_data['sessionData']['costEstimate']:.2f}")
    print(f"🎯 All Tokens: {session_data['sessionData']['realTokens']:,} (includes Claude Desktop)")
    print(f"💎 Billable: {session_data['sessionData']['billableTokens']:,}")
    ...
    # Display session type breakdown
    if 'sessionTypeBreakdown' in session_data['sessionData']:
        print(f"📊 Session Types:")
        for breakdown in session_data['sessionData']['sessionTypeBreakdown']:
            print(f"   {breakdown['type']}: {breakdown['realTokens']:,} tokens, ${breakdown['cost']:.2f}")
    
    # Generate HTML with rate limits at top and NO STALE WARNINGS
...
<body>
    ...
    <div class="main-display">
        ...
        <div class="session-breakdown" id="sessionBreakdown">
            <!-- Session type breakdown will be populated here -->
        </div>
    ...
    <script>
        // Embedded real data from SQLite database with smart updates - NO STALE WARNINGS
        ...
        class SmartDashboard {{
            ...
            init() {{
                console.log('📊 Loading smart dashboard with complete token counting...');
            ...
            loadEmbeddedData() {{
                try {{
                    ...
                    if (sessionData && rateLimitData && cacheEfficiency) {{
                        ...
                        console.log('✅ Smart dashboard updated with complete token data');
                    }} else {{
            ...
            async checkForUpdates() {{
                try {{
                    // Simple check for updates without triggering stale warnings
                    console.log('🔄 Checking for session updates...');
                    
                    // Reset inactivity timer since we're still polling
                    if (this.inactivityTimer) {{
                        clearTimeout(this.inactivityTimer);
                        this.inactivityTimer = setTimeout(() => {{
                            this.stopPolling();
                        }}, 300000); // Reset 5-minute timer
                    }}
            ...
            updateDashboard(sessionData, rateLimitData, analyticsData, cacheEfficiency) {{
                try {{
                    ...
                    this.updateSessionBreakdown(session);
                    ...
                    // Update debug info - NO STALE WARNINGS
                    document.getElementById('debugInfo').innerHTML = `
                        📊 Data Source: ${{sessionData.dataSource || 'complete_tokens'}}<br>
                        ...
                        🎯 All Sessions: ${{session.globalTotals ? session.globalTotals.totalSessions : 'N/A'}} total<br>
                        💰 Total Cost: $${{(session.costEstimate || 0).toFixed(4)}}<br>
            ...
            updateSessionBreakdown(session) {{
                const breakdown = document.getElementById('sessionBreakdown');
                if (session.sessionTypeBreakdown && session.sessionTypeBreakdown.length > 0) {{
                    breakdown.innerHTML = session.sessionTypeBreakdown.map(type => `
                        <div class="session-type">
                            <strong>${{type.type}}</strong><br>
                            ${{this.formatNumber(type.realTokens)}} tokens, $${{type.cost.toFixed(2)}}
                        </div>
                    `).join('');
                }} else {{
                    breakdown.innerHTML = '<div class="session-type">Session breakdown not available</div>';
                }}
            }}
            ...
            showError(message) {{
                ...
                debugInfo.innerHTML = `
                    <div style="color: #fca5a5;">
                        ❌ ${{message}}<br>
                        💡 This smart dashboard uses embedded real data with complete token counting
                    </div>
                `;
            }}
    ...
    with open('dashboard_smart_fixed.html', 'w') as f:
        f.write(html_content)
    
    print(f"✅ Smart dashboard with fixed warnings and complete tokens generated: dashboard_smart_fixed.html")
    return 'dashboard_smart_fixed.html'
